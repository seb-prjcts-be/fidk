...
//----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	echo "<ul><li><a href='preview_public.php?actie=toon_alles&size=60' target='preview'>Toon alles (thumbnails - 60px).</a>";
	echo "<li><a href='preview_public.php?actie=toon_alles&size=120' target='preview'>Toon alles (thumbnails - 120px).</a></ul>";
	}
//----------------------------------------------------------------------------------------------------------------------------------------------------
	if($actie=="toon_fotograaf" && isset($art_id) && $art_id != 0)
//----------------------------------------------------------------------------------------------------------------------------------------------------
	{
	// Haal fotograaf informatie op
	$sql_artist = "SELECT * FROM tbl_artists WHERE artist_pk = ".$art_id;
	$rs_artist = mysqli_query($conn1, $sql_artist);
	$artist_data = mysqli_fetch_assoc($rs_artist);
	
	// Toon fotograaf naam als titel
	echo "<H1>".$artist_data['artist_name']."</H1>";
	
	// Haal bio op indien beschikbaar
	// Eerst de naam van de fotograaf ophalen om de bio te kunnen zoeken
	$artist_name = $artist_data['artist_name'];
	$sql_bio = "SELECT * FROM tbl_artist_bio WHERE artist_bio_artist_name = '".$artist_name."'";
	$rs_bio = mysqli_query($conn1, $sql_bio);
	
	if(mysqli_num_rows($rs_bio) > 0) {
		$bio_data = mysqli_fetch_assoc($rs_bio);
		echo "<div class='artist-bio'>";
		echo "<h3>Biografie</h3>";
		echo "<p>".$bio_data['artist_bio_txt']."</p>";
		echo "</div>";
	}
	
	// Haal alle series van deze fotograaf op
	$sql_series = "SELECT * FROM tbl_series WHERE serie_artist_fk = ".$art_id." ORDER BY serie_name";
	$rs_series = mysqli_query($conn1, $sql_series);
	
	echo "<h2>Series</h2>";
	
	if(mysqli_num_rows($rs_series) > 0) {
		echo "<ul class='series-list'>";
		while($serie = mysqli_fetch_assoc($rs_series)) {
			$serie_url = "?actie=serie&ser_id=".$serie['serie_pk']."&art_id=".$art_id;
			echo "<li><a href='preview_public.php".$serie_url."'>".$serie['serie_name']."</a></li>";
		}
		echo "</ul>";
	} else {
		echo "<p>Geen series gevonden voor deze fotograaf.</p>";
	}
	
	// Hier stond voorheen de AI-update integratie
	// Deze functionaliteit is verwijderd voor betere prestaties
	}
//----------------------------------------------------------------------------------------------------------------------------------------------------
if($actie=="search_bio")
//----------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	$rs = mysqli_query($conn1,$sql1);
		...
		while ($row=mysqli_fetch_array($rs))
		{
			...
			echo "<p>".$serie_bio_artist_name." - <a href='preview_public.php?actie=serie&ser_id=".$serie_pk."&art_id=".$serie_bio_artist_fk."'>".$serie_bio_serie_name."</a></p>";
   ...
   if($is_content>0)
   {
	...
	$rs = mysqli_query($conn1,$sql1);
		while ($row=mysqli_fetch_array($rs))
		{
			...
			$artist_url = "?actie=toon_fotograaf&art_id=".$art_id."&artist_name=".$artist_namedb."&artist_folder_name=".$artist_name_url_enc;
			echo "<a href='preview_public.php".$artist_url."'>".$artist_namedb."</a>";
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
  {
	...
	$countrs=0;
		while ($row=mysqli_fetch_array($rs))
		{
			...
			echo "</td>";
			echo "<td valign='top'>";
			echo "<a href='preview_public.php?actie=toon_serie&art_id=".$artist_pk."&artist_name=".$artist_name."&artist_folder_name=".$artist_folder_name."'>".$artist_namedb."</a>";
			echo "<br>".$serie_namedb;
...
foreach($abc as $val) 
{
	if($val==$letter)
	{
		$abc_string.="<a href='preview_public.php?actie=toon_alles&size=".$size."&letter=".$val."' style='color:red;'>".$val."</a> <span style='color:#0080FF;'>&middot;</span> ";
	}
	else
	{
		$abc_string.="<a href='preview_public.php?actie=toon_alles&size=".$size."&letter=".$val."'>".$val." </a> <span style='color:#0080FF;'>&middot;</span> ";	
	}
}
	...
	$countrs=0;
		while ($row=mysqli_fetch_array($rs))
		{
			...
			if($folder_name_to_show != $serie_folder_name || $artist_to_show != $artist_name)
			{
				...
				if($size==60)
				{
					echo "<hr><p>".$artist_name_show." - <a href='preview_public.php?actie=serie&ser_id=".$serie_pk."&art_id=".$artist_pk."'>".$serie_name."</a></p>";
				}
				else
				{
					echo "<hr><h2>".$artist_name_show."<br><a href='preview_public.php?actie=serie&ser_id=".$serie_pk."&art_id=".$artist_pk."'>".$serie_name."</a></h2>";
				}
